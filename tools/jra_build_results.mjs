/* JRA の日別・場別の的中率と回収率 → data/jra/results.json（TOP の中央競馬面に出す）
   ボートの build_boat_results と同じ：jra_build_races が「発走が過ぎたレース」の予想を preds.jsonl に記録し、
   ここで results.jsonl の払戻と突き合わせる。買い方は jra_backtest と同じ（1点100円）。
     JRA_REC_DAYS … 何日ぶんを載せるか（既定 60） */
...
if (!fs.existsSync(PREDS)) { console.error('preds.jsonl がまだ無い（発走が過ぎたレースを jra_build_races が記録する）'); process.exit(0); }
const preds = new Map();
...
const K = new Map();
for (const line of fs.readFileSync(path.join(ROOT, 'data/jra/results.jsonl'), 'utf8').split('\n')) {
  const m = line.match(/^\{"raceId":"(\d{12})","date":"([\d-]+)"/);
...
function settle(p, k) {
  ...
  const h6 = t1 === f1 && [t2, t3, t4].includes(f2) && [t2, t3, t4].includes(f3); add(S, '◎1着 三連単6点', 600, h6 ? payOf(k, 'santan', e3k) : 0, h6);
  const ai = p.ai || {};
...
const T = out.total;
console.error(`${dates.length}日 ${matched}R を精算（未確定 ${pending}R）。◎的中 ${T.hit1 != null ? (T.hit1 * 100).toFixed(1) : '—'}%／◎単勝 回収 ${T.bets['◎単勝'].roi != null ? (T.bets['◎単勝'].roi * 100).toFixed(1) : '—'}%／3頭BOX三連複 ${T.bets['3頭BOX三連複'].roi != null ? (T.bets['3頭BOX三連複'].roi * 100).toFixed(1) : '—'}% -> data/jra/results.json`);
