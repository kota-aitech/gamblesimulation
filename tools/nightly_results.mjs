...
try {
  ...
  git(['commit', '-q', '-m', `chore(results): ${env.NK_BT_TO} の結果を取り込み`, '-m', 'tools/nightly_results.mjs による自動コミット（結果・払戻・最終オッズ → 日別の成績）', '--', ...TARGETS.filter(f => fs.existsSync(path.join(ROOT, f)))]);
